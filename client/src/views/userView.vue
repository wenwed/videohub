...
export default {
  ...
  created() {
    ...
    this.axios
      .get(`/user/userinfo?usid=${this.$route.params.usid}`)
      .then((res) => {
        this.userInfo = res.data.userinfo;
        return this.$axios.get(
          `/video/person/passed?id=${this.$route.params.usid}`
        );
      })
      .then((res) => {
  ...
  computed: {
    profile() {
      return (
        "http://127.0.0.1:8633/api/user/getposter?poster=" +
        this.userInfo[0].user_poster
      );
    },
    username() {
    ...
    userdescripe() {
      return this.userInfo[0].user_descripe
        ? this.userInfo[0].user_descripe
        : "这个人很懒，什么都没有写";
    },
  },
};
